...
class SDXL:
    ...
    @torch.no_grad()
    def get_text_embed(
        self, null_prompt_1, prompt_1, null_prompt_2=None, prompt_2=None, clip_skip=None
    ):
        """
    ...
    def differentiable_get_text_embed(
        self,
        null_prompt_1,
        prompt_1,
        null_prompt_2=None,
        ...
        if te1_nograd:
            ...
            prompt_embed_1, pool_prompt_embed = self._differentiable_text_embed(
                prompt_1, self.tokenizer_1, self.text_enc_1, clip_skip
            )
        ...
        if prompt_2 is None:
            ...
            if te2_nograd:
                ...
                prompt_embed_2, pool_prompt_embed = self._differentiable_text_embed(
                    prompt_2, self.tokenizer_2, self.text_enc_2, clip_skip
                )
            prompt_embed = [prompt_embed_1, prompt_embed_2]
        ...
        if te1_nograd:
            ...
            null_embed_1, pool_null_embed = self._differentiable_text_embed(
                null_prompt_1, self.tokenizer_1, self.text_enc_1, clip_skip
            )
        ...
        if null_prompt_2 is None:
            ...
            if te2_nograd:
                ...
                null_embed_2, pool_null_embed = self._differentiable_text_embed(
                    null_prompt_2, self.tokenizer_2, self.text_enc_2, clip_skip
                )
            null_embed = [null_embed_1, null_embed_2]
    ...
    def sample(
        ...
        **kwargs,
        ...
        # embedding
        (null_prompt_embeds, prompt_embeds, pool_null_embed, pool_prompt_embed) = (
            self.get_text_embed(
    ...
    def initialize_latent(
        ...
        **kwargs,
        ...
        if method == "ddim":
            ...
            z = self.inversion(
                self.encode(src_img.to(self.dtype).to(self.device)),
                kwargs.get("uc"),
                kwargs.get("c"),
    ...
    def prompt_opt(
        ...
        para = list(self.text_enc_1.get_input_embeddings().parameters()) + list(
            self.text_enc_2.get_input_embeddings().parameters()
        )
        optimizer = Adam(para, lr=popt_kwargs["p_opt_lr"] * (1.0 - step * decay_rate))

        # keep original embeddings as reference
        orig_embeds_params_enc1 = (
            self.text_enc_1.get_input_embeddings().weight.data.clone()
        )
        orig_embeds_params_enc2 = (
            self.text_enc_2.get_input_embeddings().weight.data.clone()
        )
        ...
        if popt_kwargs["placeholder_position"] == "end":
            prompt_list_1 = [
                p
                + " "
                + " ".join(f"*_{num_opt_tokens*idx+i}" for i in range(num_opt_tokens))
                for idx, p in enumerate(prompt_list_1)
            ]
        ...
        if popt_kwargs["placeholder_position"] == "end":
            prompt_list_2 = [
                ...
                + " "
                + " ".join(f"*_{num_opt_tokens*idx+i}" for i in range(num_opt_tokens))
                for idx, p in enumerate(prompt_list_2)
            ]
        elif popt_kwargs["placeholder_position"] == "start":
            prompt_list_2 = [
                " ".join(f"*_{num_opt_tokens*idx+i}" for i in range(num_opt_tokens))
        ...
        add_cond_kwargs_base["text_embeds"] = (
            add_cond_kwargs_base["text_embeds"][-1].unsqueeze(0).detach()
        )
        add_cond_kwargs_base["time_ids"] = (
    ...
    def get_embed_from_prompt12(
        self, prompt1, prompt2, te1_nograd=False, te2_nograd=False
    ):
        # 0. Default height and width to unet
        height = self.default_sample_size * self.vae_scale_factor
        width = self.default_sample_size * self.vae_scale_factor
        ...
        # embedding
        (null_prompt_embeds, prompt_embeds, pool_null_embed, pool_prompt_embed) = (
            self.differentiable_get_text_embed(
                prompt1[0],
                prompt1[1],
                prompt2[0],
                prompt2[1],
    ...
    def restore_embedding(
        self, placeholder_token_ids, orig_embeds_params, tokenizer, text_enc
    ):
        ...
        with torch.no_grad():
            text_enc.get_input_embeddings().weight[index_no_updates] = (
                orig_embeds_params[index_no_updates]
            )

    def initialize_embedding(self, tokenizer, text_enc, popt_kwargs, b_size=1):
        num_opt_tokens = (
            popt_kwargs["num_opt_tokens"] * b_size
        )  # assignging popt_kwargs['num_opt_tokens'] tokens per each sample
        ...
        assert (
            "_" in placeholder_string and len(placeholder_string.split("_")) == 2
        )  # the tokens should take the form of "*_0"
        ...
        if init_type == "word":
            assert (init_rand_vocab != init_max_cs) or (
                init_rand_vocab == False and init_max_cs == False
            )
            if init_rand_vocab:
                ...
                with torch.no_grad():
                    for token_id in placeholder_token_ids:
                        ...
                        print(
                            f"Initialize token id {token_id} as a random vocabulary of index {rand_idx}."
                        )
                        token_embeds[token_id] = token_embeds[rand_idx].clone()
            elif init_max_cs:
                assert (
                    self.prompt1 == self.prompt2
                )  # for now, we only support the same prompt for both encoders
                # get rid of indices of special tokens in token_embeds_base
                special_token_ids = torch.tensor(tokenizer.all_special_ids)
                token_embeds_min_cs = token_embeds_base[
                    ~torch.isin(
                        torch.arange(token_embeds_base.shape[0]), special_token_ids
                    )
                ]

                prompt1 = self.prompt1.copy()
                prompt1_ids = tokenizer.encode(
                    prompt1[1], add_special_tokens=False, return_tensors="pt"
                ).squeeze()
                prompt1_embeds = token_embeds_min_cs[prompt1_ids]
                cos_sims = torch.einsum(
                    "ij,kj->ik", token_embeds_min_cs, prompt1_embeds
                ).sum(dim=-1)
                ...
                with torch.no_grad():
                    for token_id in placeholder_token_ids:
                        print(
                            f"Initialize token id {token_id} as a min-cs vocabulary of index {min_idx}."
                        )
                ...
                with torch.no_grad():
                    for token_id in placeholder_token_ids:
                        print(
                            f"Initialize token id {token_id} as the token embeddin of {init_word}."
                        )
            ...
            if init_type == "gaussian_white":
                var_vector = (
                    token_embeds_base**2 - embeds_mean.unsqueeze(0) ** 2
                ).mean(dim=0)
                embeds_cov = torch.diag(var_vector) * (init_gau_scale**2)
            elif init_type == "gaussian":
                embeds_cov = (
                    torch.einsum("ij,ik->jk", token_embeds_base, token_embeds_base)
                    / token_embeds_base.shape[0]
                )
            ...
            mvn = torch.distributions.MultivariateNormal(
                embeds_mean, covariance_matrix=embeds_cov
            )
            ...
            with torch.no_grad():
                for token_id in placeholder_token_ids:
                    print(
                        f"Initialize token id {token_id} as a multivariate normal distribution ({init_type})."
                    )
...
class SDXLLightning(SDXL):
    def __init__(
        ...
        pipe = StableDiffusionXLPipeline.from_pretrained(
            base_model_key, unet=self.unet, torch_dtype=dtype
        ).to(device)
        self.dtype = dtype
        ...
        **kwargs,
        ...
        zt = self.initialize_latent(
            size=(
                1,
                4,
                shape[1] // self.vae_scale_factor,
                shape[0] // self.vae_scale_factor,
            )
        )
        ...
        if popt_kwargs["prompt_opt"]:
            ...
            placeholder_token_ids_enc1 = self.initialize_embedding(
                self.tokenizer_1, self.text_enc_1, popt_kwargs
            )
            placeholder_token_ids_enc2 = self.initialize_embedding(
                self.tokenizer_2, self.text_enc_2, popt_kwargs
            )
        ...
        for step, t in enumerate(pbar):
            ...
            # for prompt-opt
            if (
                popt_kwargs["prompt_opt"]
                and t > popt_kwargs["t_lo"] * len(self.scheduler.alphas_cumprod_default)
    ...
    def sample(
        ...
        **kwargs,
        ...
        (
            ...
            src_prompt_embeds,
            pool_null_embed,
            pool_src_prompt_embed,
        ) = self.get_text_embed(
            prompt1[0], prompt1[1], prompt2[0], prompt2[1], clip_skip
        )

        (_, tgt_prompt_embeds, _, pool_tgt_prompt_embed) = self.get_text_embed(
            prompt1[0], prompt1[2], prompt2[0], prompt2[2], clip_skip
        )
        ...
        if cfg_guidance != 0.0 and cfg_guidance != 1.0:
            # do cfg
            add_src_text_embeds = torch.cat(
                [negative_text_embeds, add_src_text_embeds], dim=0
            )
            add_tgt_text_embeds = torch.cat(
                [negative_text_embeds, add_tgt_text_embeds], dim=0
            )
            add_time_ids = torch.cat([negative_add_time_ids, add_time_ids], dim=0)
    ...
    def reverse_process(
        ...
        **kwargs,
        ...
        for step, t in enumerate(pbar):
            ...
            with torch.no_grad():
                noise_uc, noise_c = self.predict_noise(
                    zt, t, null_prompt_embeds, tgt_prompt_embed, add_tgt_cond_kwargs
                )
                noise_pred = noise_uc + cfg_guidance * (noise_c - noise_uc)
