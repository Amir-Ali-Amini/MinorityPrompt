...
class EvaluationResult:
    ...
    def save(self, output_dir: Union[str, Path], prefix: str = ""):
        ...
        if self.metrics:
            # Summary
            self.metrics.summary().to_csv(
                output_dir / f"{prefix}metrics_summary.csv", index=False
            )
            ...
            if len(self.metrics.bias_p) > 0:
                self.metrics.bias_p.to_csv(
                    output_dir / f"{prefix}bias_p.csv", index=False
                )
...
class DemographicEvaluator:
    ...
    def evaluate_images(
        ...
        t0 = time.time()
        predictions = self.demographic_predictor.predict_faces(
            all_faces, progress=progress
        )
        result.prediction_time = time.time() - t0

        # Convert to DataFrame
        result.predictions_df = self.demographic_predictor.predictions_to_dataframe(
            predictions
        )

        # Add original image column for Bias-P calculation
        result.predictions_df["original_image"] = result.predictions_df[
            "face_name_align"
        ].apply(
            lambda x: (
                os.path.basename(x).split("_face")[0] if isinstance(x, str) else str(x)
            )
        )
    ...
    def evaluate_tensors(
        ...
        try:
            ...
            for i, tensor in enumerate(tensors):
                ...
                img_path = temp_dir / f"image_{i:04d}.png"
                save_image(tensor, img_path)
                image_paths.append(str(img_path))

            if progress:
                print(f"Saved {len(image_paths)} tensors to temp directory")

            # Evaluate using file paths (the working method)
            return self.evaluate_images(
                images=image_paths,
                tag=tag,
                output_dir=output_dir,
    ...
    def print_comparison(
        ...
        print(f"\nFaces detected:")
        print(
            f"  {result_a.tag}: {result_a.num_faces} faces from {result_a.images_with_faces}/{result_a.num_images} images"
        )
        print(
            f"  {result_b.tag}: {result_b.num_faces} faces from {result_b.images_with_faces}/{result_b.num_images} images"
        )
        ...
        for _, row in comparison.iterrows():
            ...
            print(
                f"  {attr:25s}: {a_val:.4f} → {b_val:.4f} ({direction} {abs(diff):.4f})"
            )
        ...
        for _, row in comparison.iterrows():
            ...
            direction = "↑" if diff > 0 else "↓" if diff < 0 else "="
            print(
                f"  {attr:25s}: {a_val:.2f} → {b_val:.2f} ({direction} {abs(diff):.2f})"
            )
