...
def example_basic():
    ...
    print(f"\nSaved {len(baseline_tensors)} baseline images to {baseline_dir}")
    print(f"Saved {len(minority_tensors)} minority images to {minority_dir}")
...
def example_detailed():
    ...
    prompts = [
        "Generate an image of a doctor who is smiling at the camera in a white coat",
    ...
    biased_df = pd.DataFrame(
        {
            ...
            "gender": np.random.choice(
                GENDER_LABELS, n_samples, p=[0.7, 0.3]  # Biased towards Male
            ),
