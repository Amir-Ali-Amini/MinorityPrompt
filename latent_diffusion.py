...
class StableDiffusion:
    def __init__(
        ...
        **kwargs,
        ...
        self.scheduler.alphas_cumprod_default = (
            self.scheduler.alphas_cumprod_default.to(device)
        )
    ...
    def get_text_embed(self, null_prompt, prompt):
        ...
        null_text_input = self.tokenizer(
            null_prompt,
            padding="max_length",
    ...
    def predict_noise(
        self, zt: torch.Tensor, t: torch.Tensor, uc: torch.Tensor, c: torch.Tensor
    ):
        """
    ...
    def prompt_opt(
        ...
        if popt_kwargs["placeholder_position"] == "end":
            prompt_list = [
                ...
                + " "
                + " ".join(
                    f"{placeholder_symbol}_{num_opt_tokens*idx+i}"
                    for i in range(num_opt_tokens)
                )
        ...
        uc, c = self.differentiable_get_text_embed(
            null_prompt=prompt[0], prompt=prompt[1]
        )
    ...
    def batch_prompt_opt(
        ...
        _, c = self.differentiable_get_text_embed(
            null_prompt=null_prompts, prompt=prompts
        )
    ...
    def initialize_embedding(self, tokenizer, text_enc, popt_kwargs, b_size=1):
        ...
        if "gaussian" in init_type:
            token_embeds_base = (
                text_enc.get_input_embeddings().weight.data.detach().clone()
            )
    ...
    def batch_sample(
        ...
        **kwargs,
        ...
        for step, t in enumerate(pbar):
            ...
            at = self.scheduler.alphas_cumprod[ts].view(b_size, 1, 1, 1)
            at_prev = self.scheduler.alphas_cumprod[ts - self.skip].view(
                b_size, 1, 1, 1
            )
            ...
            if (
                ...
                if (
                    popt_kwargs["popt_diverse"]
                    and popt_kwargs["base_prompt_after_popt"]
                ):
                    c = c_base.detach().clone()
...
class InversionDDIM(BaseDDIM):
    ...
    @torch.autocast(device_type="cuda", dtype=torch.float16)
    def sample(
        self, src_img, cfg_guidance=7.5, prompt=["", "", ""], callback_fn=None, **kwargs
    ):
        ...
        # Initialize zT
        zt = self.initialize_latent(
            method="ddim", src_img=src_img, uc=uc, c=c, cfg_guidance=cfg_guidance
        )
        zt = zt.requires_grad_()
...
class EditWardSwapDDIM(InversionDDIM):
    ...
    def sample(
        ...
        # Initialize zT
        zt = self.initialize_latent(
            method="ddim", src_img=src_img, uc=uc, c=src_c, cfg_guidance=cfg_guidance
        )
        # Sampling
